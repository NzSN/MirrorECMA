...
import { ReplayMismatchError, replayReport, type ReplayRecorder, type CompiledReplayReport as ReplayReport } from "./replay-report.js";
  ...
  for (;;) {
    ...
    switch (msg.proto_step) {
      case "initial_state": {
        ...
        traceIndex += 1;
        stepIndex = 0;
        if (options.progress) lastParam = structuredClone(msg.state);
      ...
      case "next_step": {
        ...
        lastAction = msg.action;
        stepIndex += 1;
        lastParam = options.progress ? structuredClone(msg.parameters) : msg.parameters;
      ...
      case "step_mismatch": {
        ...
        if (options.progress) {
          ...
          throw new ReplayMismatchError(action, lastParam, msg.expected, msg.actual, msg.hints,
            position.traceIndex, position.stateIndex);
        }
