...
import { createVerifyRequest, encodeModelInterfaceRegistration } from "./model-interface.js";
...
async function loadGateSdk(): Promise<GateSdk> {
  try {
...
async function connectGate(
  ...
  const common = { requiredCapabilities: [...capabilities] };
  try {
...
async function waitSucceeded<T>(
  ...
  const registry = new AsyncCompiledAdapterRegistry([{
    ...
    factory: async (config, authority) => {
      ...
      try {
        ...
        const binding = await prepared.model.createBinding(port, config);
        throwIfFactoryInactive(authority);
        return Object.freeze({
          ...
          dispose: async () => {
            ...
            port!.close();
            let primary: unknown;
            try { await binding.dispose(); } catch (error) { primary = error; }
            try { await worker!.close(); } catch (error) { if (primary === undefined) primary = error; }
            if (primary !== undefined) throw primary;
          },
        });
      } catch (error) {
        ...
        port?.close();
        if (worker !== undefined) {
          try { await worker.close(); } catch { /* Gate session close is the cleanup backstop. */ }
        } else if (reservation !== undefined) {
          try {
            await waitSucceeded(await reservation.release("client-failure"), undefined, deadlines.receiveMs);
          } catch { /* Gate session close below joins any partial release. */ }
        }
        throw error;
      }
    },
...
function errorFamily(error: unknown): SandboxFailureFamily {
...
function isCancelled(error: unknown): boolean {
  return chainHas(error, (item) => item.name === "AbortError" || item.name === "ReplayCancelledError" ||
    item.code === "CANCELLED" || item.code === "CONTROL_WAIT_CANCELLED" || item.code === "operation_cancelled");
}
...
function cleanupTag(cleanup: GateCleanup | undefined, clientClosed: boolean): SandboxCleanupStatus {
