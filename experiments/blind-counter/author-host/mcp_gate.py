...
TOOLS = [
    {
        ...
        "description": "Read the complete approved public Counter implementation contract.",
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
    },
    {
        "name": "gate_exec",
        "description": (
            "Execute an approved interpreter inside Gate's restricted authoring sandbox. "
            "The current directory is the submission workspace. Every command, file read, "
            "file write, and public test must use this tool. Select author.python with "
            "Python arguments or author.node with Node arguments. No host path or policy "
            "can be selected. Nonzero exit codes return bounded public diagnostics."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "toolId": {"type": "string", "enum": ["author.python", "author.node"]},
                "args": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 128},
            },
    ...
    {
        "name": "submit",
        "description": "Irrevocably finish authoring and submit the current source for freezing and private evaluation. No further authoring commands are accepted.",
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
    },
]
        ...
        try:
            if method == "initialize":
                response["result"] = {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "gate-author-only", "version": "1.0.0"},
                }
            elif method == "ping":
                response["result"] = {}
                ...
                call = {"id": counter}
                if name == "gate_exec":
                    if set(arguments) != {"toolId", "args"} or arguments["toolId"] not in ("author.python", "author.node"):
                        raise ValueError("unknown authoring operation")
                    args = arguments["args"]
                    if not isinstance(args, list) or not 1 <= len(args) <= 128 or any(not isinstance(x, str) or not x or "\0" in x for x in args):
