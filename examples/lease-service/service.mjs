...
export class LeaseService {
  ...
  valid(client, token) {
    return (
      this.owners.has(client) && token === this.epoch && this.now < this.expires
    );
  }
  acquire(client) {
    ...
    if (this.accepted) {
      this.owners = new Set([client]);
      this.epoch++;
      this.expires = this.now + 3n;
    }
  }
  renew(client, token) {
    this.accepted =
      this.fault === "invalid-renewal" || this.valid(client, token);
  ...
  write(client, token) {
    this.accepted =
      this.fault === "expired-token"
        ? this.owners.has(client) && token === this.epoch
        : this.valid(client, token);
