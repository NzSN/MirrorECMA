...
assert(['persistent-transfer', 'lease-service'].includes(folder), 'Use the existing WorkQueue acceptance CLI for work-queue');
...
for (let i = 0; i < args.length; i++) {
  ...
  else throw new Error('Usage: node run.mjs persistent-transfer|lease-service [--live] [--receipt NEW_FILE]');
}
