...
import { evaluateSandboxedWithDependencies } from "../src/sandbox.js";
import {
...
function realGateDependencies(probe: GateProbe) {
  return {
    loadGateSdk: async () => {
      const control = await import(CONTROL_SPECIFIER) as any;
      const workerSdk = await import(WORKER_SPECIFIER) as any;
      ...
      return {
        ControlClient: {
          launch: async (...args: any[]) => wrapClient(await control.ControlClient.launch(...args)),
          connectUnix: async (...args: any[]) => wrapClient(await control.ControlClient.connectUnix(...args)),
        },
        createPublicManifest: workerSdk.createPublicManifest,
        toWorkerValue: workerSdk.toWorkerValue,
        fromWorkerValue: workerSdk.fromWorkerValue,
      };
    },
  };
}
...
async function main(): Promise<void> {
  ...
  const options = {
    ...
    onDiagnostic: (diagnostic: import("../src/index.js").TrustedSandboxDiagnostic) => {
  ...
  if (scenario === "cancel") {
    ...
    result = await evaluateSandboxedWithDependencies(
      plan, options, realGateDependencies(probe) as never,
    );
  } else {
    result = await evaluateSandboxed(plan, options);
  }
